    ...
    <div class="col-span-1">
        <div class="flex items-center justify-center">
            <form action="{{ route('dashboard') }}" method="POST" class="med">
                @csrf
                <select id="choices-multiple-remove-button" class="med" name=conditions[] placeholder="Select upto 5 medical conditions" multiple value="HTML">
                    @foreach ($tags as $tag)
                    {{ $flag = true }}
                ...
                <button class="med my-2 group relative w-full flex justify-center py-2 px-4 border border-transparent text-lg font-bold font-medium rounded-md text-white button-color hover:button-color focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-300" type="submit">
                    Save
                </button>
            </form>
        </div>
...
<div id="rec" class="cards">
    ...
    <div class="flex justify-between m-6">
        <div class="relative flex flex-col h-full max-w-md mx-auto bg-gray-50 rounded-lg">
            <img class="rounded-lg rounded-b-none" src="http://www.3forty.media/ruki/wp-content/uploads/2020/06/meditation-yoga-1024x682.jpg" alt="thumbnail" loading="lazy" />

            <div class="py-2 px-4">
                <h1 class="text-xl font-medium leading-6 tracking-wide text-gray-800 hover:text-blue-500 cursor-pointer">
                    <a href="{{ route('asanas', ['AID' => $recommendation->AID]) }}">{{ $recommendation->AName }}</a>
                </h1>
            </div>
            <div class="px-4 space-y-2">
                ...
                <a href="{{ route('asanas', ['AID' => $recommendation->AID]) }}" to="blog/detail" class="font-bold hover:text-blue-400 text-blue-400">read more...
                </a>
            </div>
            <div class="flex flex-row items-end h-full w-full px-4 mt-4 btn--block">
                <a href="{{ route('asanas', ['AID' => $recommendation->AID]) }}"><button class="btn btn--block card__btn">Open</button></a>
            </div>
        </div>
    </div>
    @endforeach
    ...
    <div class="flex justify-between m-6">
        <div class="relative flex flex-col h-full max-w-md mx-auto bg-gray-50 rounded-lg">
            ...
            <div class="px-4 space-y-2">
                ...
                <a href="{{ route('asanas', ['AID' => $asana->AID]) }}" to="blog/detail" class="font-bold hover:text-blue-400 text-blue-400">read more...
                </a>
            </div>
            <div class="flex flex-row items-end h-full w-full px-4 mt-4 btn--block">
                <a href="{{ route('asanas', ['AID' => $asana->AID]) }}"><button class="btn btn--block card__btn">Open</button></a>
            </div>
        </div>
    </div>
    @endforeach
